...
app.get("/api/events", (req, res) => {
    ...
    const fullEvents = events.map(event => {
        const dates = db.prepare("SELECT * FROM dates WHERE event_id = ?").all(event.id);
        const fullDates = dates.map(date => ({
            ...date,
            records: db.prepare("SELECT * FROM answers WHERE date_id = ?").all(date.id)
        }));
...
app.post("/api/events", (req, res) => {
    ...
    const info = db.prepare("INSERT INTO events (title, location) VALUES (?, ?)").run(title, location);
    const eventId = info.lastInsertRowid;

    dates?.forEach((d: any) => {
        db.prepare("INSERT INTO dates (event_id, timestamp) VALUES (?, ?)").run(eventId, d.timestamp);
